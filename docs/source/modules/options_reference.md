...
The option `USE_USERDEF_SHOCK_FLATTENING` calls the user routine `userdef_shock_flattening(mgrid,lgrid)` defined in `app.F90`, letting the application decide which cells are marked through `is_flattened`. Cells with `is_flattened = 1` apply the the shock-flattening procedure during the whole simulation. It can be used on its own, or together with `USE_SHOCK_FLATTENING` if you also want shock detection in the interior. This option only applies to cells selected by the application routine. See the `kh_userdef_flattening` test for an example of how the routine should be constructed.
